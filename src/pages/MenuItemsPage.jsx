...
export default function MenuItemsPage() {
    ...
    const styles = `
        ...
        .hero-title { font-family: 'Playfair Display', serif; font-size: 3rem; font-weight: 700; color: #f5edd8; margin-bottom: 0.5rem; }
        ...
        .dish-emoji { width: 72px; height: 72px; border-radius: 8px; background: rgba(255,255,255,0.05); display: flex; align-items: center; justify-content: center; font-size: 28px; flex-shrink: 0; }
        ...
        .tag { font-size: 10px; font-weight: 500; letter-spacing: 0.08em; text-transform: uppercase; padding: 2px 8px; border-radius: 20px; margin-top: 8px; display: inline-block; margin-right: 4px; }
        .tag-available { background: rgba(100,180,100,0.15); color: #7ec87e; border: 0.5px solid rgba(100,180,100,0.4); }
        .tag-unavailable { background: rgba(220,80,80,0.15); color: #e08080; border: 0.5px solid rgba(220,80,80,0.4); }
        .dish-expanded { background: rgba(255,255,255,0.05); border-radius: 8px; padding: 1rem; margin-top: 0.75rem; font-size: 13px; color: rgba(245,237,216,0.6); }
    ...
    return (
        <div style={{ fontFamily: "'DM Sans', sans-serif", minHeight: '100vh', background: '#1a1208' }}>
            ...
            <div className="menu-hero">
                <div className="hero-eyebrow">Göteborg · Sedan 1987</div>
                <h1 className="hero-title">Trattoria Bella</h1>
                <div className="hero-divider">✦</div>
                <div className="hero-sub">Autentiskt kök från Toscana — tillagat med kärlek varje dag</div>
            </div>
...
function DishRow({ item, open, onToggle }) {
    const emoji = categoryEmojis[item.categoryId] || '🍽️'
    return (
        <div className="dish-row" onClick={onToggle}>
            ...
            <div className="dish-content">
                ...
                <span className={`tag ${item.isAvailable ? 'tag-available' : 'tag-unavailable'}`}>
                    {item.isAvailable ? 'Tillgänglig' : 'Slut'}
                </span>
                {open && (
                    <div className="dish-expanded">
                        <strong>Kategori-ID:</strong> {item.categoryId}
                    </div>
                )}
            </div>
        </div>
